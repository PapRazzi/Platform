...
import com.alliander.osgp.adapter.ws.loadmanagement.application.exceptionhandling.DetailSoapFaultMappingExceptionResolver;
import com.alliander.osgp.adapter.ws.loadmanagement.application.exceptionhandling.SoapFaultMapper;
import com.alliander.osgp.shared.application.config.AbstractConfig;

@Configuration
@PropertySources({
	@PropertySource("classpath:osgp-adapter-ws-loadmanagement.properties"),
...
public class WebServiceConfig extends AbstractConfig {
    ...
    private static final String PROPERTY_NAME_MARSHALLER_CONTEXT_PATH_LOAD_MANAGEMENT_AD_HOC_MANAGEMENT = "jaxb2.marshaller.context.path.loadmanagement.adhocmanagement";
     ...
     */
    @Bean
    public Jaxb2Marshaller loadManagementAdHocManagementMarshaller() {
        LOGGER.debug("Creating Load Management Ad Hoc Management JAXB 2 Marshaller Bean");

        final Jaxb2Marshaller marshaller = new Jaxb2Marshaller();

        marshaller.setContextPath(this.environment
                .getRequiredProperty(PROPERTY_NAME_MARSHALLER_CONTEXT_PATH_LOAD_MANAGEMENT_AD_HOC_MANAGEMENT));

        return marshaller;
    }

    /**
     * Method for creating the Marshalling Payload Method Processor for load
     * management schedule management.
     *
     * @return MarshallingPayloadMethodProcessor
     */
    @Bean
    public MarshallingPayloadMethodProcessor loadManagementAdHocManagementMarshallingPayloadMethodProcessor() {
        LOGGER.debug("Creating Load Management Ad Hoc Management Marshalling Payload Method Processor Bean");

        return new MarshallingPayloadMethodProcessor(this.loadManagementAdHocManagementMarshaller(),
                this.loadManagementAdHocManagementMarshaller());
    }
    ...
    public DefaultMethodEndpointAdapter defaultMethodEndpointAdapter() {
        ...
        // Add Load Management Marshalling Payload Method Processors to Method
        // Argument Resolvers
        methodArgumentResolvers.add(this.loadManagementAdHocManagementMarshallingPayloadMethodProcessor());
        ...
        methodReturnValueHandlers.add(this.loadManagementAdHocManagementMarshallingPayloadMethodProcessor());
